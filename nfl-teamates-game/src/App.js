...
function CommonPlayerGame() {
    const [currentIndex] = useState(0); // Removed setCurrentIndex since it's never used
...
export default function App() {
  useEffect(() => {
    async function fetchPlayers() {
      try {
        await getPlayers(); // Call getPlayers but don't store result since it's not used
      } catch (error) {
